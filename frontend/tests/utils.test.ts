...
describe("linkResolver", () => {
  ...
  it("resolves href links", () => {
    const link = { _type: "link" as const, linkType: "href" as const, href: "https://example.com" };
    expect(linkResolver(link)).toBe("https://example.com");
  });

  it("resolves page links", () => {
    const link = { _type: "link" as const, linkType: "page" as const, page: "about" };
    expect(linkResolver(link)).toBe("/about");
  });

  it("resolves post links", () => {
    const link = { _type: "link" as const, linkType: "post" as const, post: "hello-world" };
